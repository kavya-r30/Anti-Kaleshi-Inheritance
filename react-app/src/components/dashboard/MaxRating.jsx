    ...
    const platforms = [
      {
        ...
        status: "Top " + (data?.leetcode?.contests?.stats?.topPercentage).toString() + "%" || "",
        icon: leetcode,
      },
      {
        name: "CodeChef",
        ...
        color: "#5B4638",
        ranking: (data?.codechef?.profile?.globalRank).toLocaleString() || "",
        status: data?.codechef?.profile?.stars || "My Star★",
        icon: codechef,
      },
      {
        name: "Codeforces",
        ...
        color: "#1F8ACB",
        ranking: "",
        status: (data?.codeforces?.profile?.rank ? data?.codeforces?.profile?.rank.charAt(0).toUpperCase() 
    ...
    <div>
      <div className="flex items-center justify-between pb-0.5 mb-2">
        <div className="flex items-center space-x-2">
          <div className='w-5 h-5'><img src={platform.icon} alt="icon" /></div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className={`text-[0.925rem] px-2 py-1 rounded-full`} style={{ backgroundColor: `${platform.color}15`, color: platform.color }}>
            {platform.status}
          </div>
        </div>
      </div>
